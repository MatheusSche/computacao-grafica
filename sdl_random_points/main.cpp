...
void display()
{
    ...
    for (int i=0; i<=200; i++){
        aleatorioX = rand()%(maiorX-menorX+1) + menorX;
        aleatorioY = rand()%(maiorY-menorY+1) + menorY;

        aleatorioR = rand()%(255-0+1);
        aleatorioG = rand()%(255-0+1);
        aleatorioB = rand()%(255-0+1);
        setPixel( aleatorioX, aleatorioY, RGB(aleatorioR,aleatorioG,aleatorioB) );
    }
